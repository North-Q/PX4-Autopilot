...
PeripheralHandleMapType analogInputMap[(IO_ANALOGINPUT_MODULES_MAX/8)+((IO_ANALOGINPUT_MODULES_MAX%8)!=0)] = {0};
#else
...
void readResultAnalogInSingle(uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint16_T* peripheralDataSizeResponse, uint8_T* status)
{
    ...
    uint32_T localBuffer[(responsePeripheralPayloadSize >> 2) + 1];
...
#if ESB_BETA2_IOSERVER
        MW_AnalogIn_ReadResult((MW_Handle_Type)analogInputHandle, (void *)data, (MW_AnalogIn_ResultDataType_Type)resultDataType);
#else
        MW_AnalogInSingle_ReadResult((MW_Handle_Type)analogInputHandle, (void *)data, (MW_AnalogIn_ResultDataType_Type)resultDataType);
#endif
        /* Copy the read value to Tx Buffer */
        memcpy(&payloadBufferTx[*peripheralDataSizeResponse], data, responsePeripheralPayloadSize);
