...
// ACKNOWLEDGE_BYTE is not used as a result commenting
...
PeripheralHandleMapType i2cMap[(IO_I2C_MODULES_MAX/8)+((IO_I2C_MODULES_MAX%8)!=0)] = {0};
#else
    ...
    if ((MW_Handle_Type)NULL != i2cHandle) {
        if (_7_BIT_ADDRESSING) {
            ...
            while (slaveAddress <= MAXIMUM_USABLE_7BIT_I2CADDRESS_119) {
                ...
                *status = MW_I2C_MasterWrite(i2cHandle, slaveAddress, &data, dataLength, NO_REPEATED_START, DONT_ACKNOWLEDGE_BYTE);
...
void rawI2CWrite(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    memcpy(&slaveAddress, &payloadBufferRx[index], sizeof(uint16_T));
    index += sizeof(uint16_T);
    
    /* Reads 4 byte data length from the input buffer */
    memcpy(&numBytesToWrite, &payloadBufferRx[index], sizeof(uint32_T));
    index += sizeof(uint32_T);
    
    /* Defines a local buffer, copy data and use the pointer to local buffer to avoid alignment issues */
    uint32_T localBuffer[(numBytesToWrite >> 2) + 1];
    data = (uint8_T *)localBuffer;
    memcpy(data, &payloadBufferRx[index], numBytesToWrite);
...
void rawI2CRead(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    /* Reads 4 byte data length from the input buffer */
    memcpy(&numBytesToRead, &payloadBufferRx[index], sizeof(uint32_T));
    index += sizeof(uint32_T);
    
    /* Defining a local buffer and using the pointer to that buffer to avoid alignment issues */
    uint32_T localBuffer[(numBytesToRead >> 2) + 1];
    data = (uint8_T *)localBuffer;
    ...
    { 
        ...
        if (slaveAddress >= MINIMUM_USABLE_7BIT_I2CADDRESS_08 && slaveAddress <= MAXIMUM_USABLE_7BIT_I2CADDRESS_119) 
        {
            ...
            *status = MW_I2C_MasterRead(i2cHandle, slaveAddress, data, numBytesToRead, NO_REPEATED_START, DONT_ACKNOWLEDGE_BYTE);
            /* Copy the data read to Tx Buffer */
            memcpy(&payloadBufferTx[*peripheralDataSizeResponse], data, numBytesToRead);
...
void setI2CFrequency(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    { 
        ...
        memcpy(&i2cBusFrequency, &payloadBufferRx[index], sizeof(uint32_T));
