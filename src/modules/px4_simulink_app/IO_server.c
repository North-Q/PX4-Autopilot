...
#if IO_AUTOGEN_ENABLE
#include "iobuilder.h"
static RT_MODEL_iobuilder_T iobuilder_M_;
static RT_MODEL_iobuilder_T *const iobuilder_M = &iobuilder_M_;
#endif
...
void getHeapsize(uint8_T *payloadBufferTx, uint16_T *peripheralDataSizeResponse,uint8_T *status)
{
    ...
    while(1)
    {
        ...
        {
            free(heapPointer);
            /* On SAM and SAMD boards, malloc has bug. It allocates memory more than available on the target. Capping the value to 2048 bytes. */
...
void getMaximumPacketSize(uint8_T *payloadBufferTx, payloadSize_T *peripheralDataSizeResponse, uint8_T *status)
{
    payloadSize_T maxPacketSize = (payloadSize_T)MAX_PACKET_SIZE;
    #if 4 == IO_PAYLOADFIELDSIZE
    unpack32BitstoByte(maxPacketSize,payloadBufferTx);
    #elif 2 == IO_PAYLOADFIELDSIZE
    byteUnpack16Bits(payloadBufferTx,&maxPacketSize);
    #elif 1 == IO_PAYLOADFIELDSIZE
    memcpy(payloadBufferTx,&maxPacketSize,1);
    #endif
    *peripheralDataSizeResponse = (sizeof(payloadSize_T)*MEMORY_ADJUSTMENT);
    *status=0;
}

void initErrorPacker(void)
{
    ErrorPacket.payloadSize = UNIQUEID_SIZE + REQUESTID_SIZE + ISRAWREAD_SIZE + STATUS_SIZE;           /* No payload */
    ...
    crcMallocFailedStreamCon = calculateChecksum((simIOpacket*)&ErrorPacket,0,0xFF); /* Error packet is independent of Status field*/
    #else
    ...
    while(ptrTempNode != 0)
    {
        if(ptrTempNode->packet.isRawRead)
        {
            ...
            while(ptrTempNode->buffer->tail->status == (bufferStatusType)BUFFER_FULL)
            {
                ...
                // For soft real time schedulers, send all the packets and then exit the loop
                #ifndef SOFT_REALTIME_SCHEDULING
                return 1;
                #endif
            }
        }
...
{
    return (requestId<0x00FF);
}

uint8_T isRequestIdAutoGenerated(uint16_T requestId){
...
uint8_T processRequest(simIOpacket *packet)
{
    ...
    else if(isRequestIdAutoGenerated((uint16_T)packet->requestId)){
        #if IO_AUTOGEN_ENABLE
        uint16_T _requestId = (packet->ptrRxPayLoad[1]<<8)|(packet->ptrRxPayLoad[0]);
        autoGeneratedPeripherals(iobuilder_M,(uint8_T*)&packet->ptrRxPayLoad[2],(uint16_T)_requestId,(uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize, (uint8_T*)&packet->status,(uint8_T*)&funNotFound);
        #else
        funNotFound = 1;
        #endif 
    }
